...
data:
  ...
  alertrules.yml: |
    ...
    - name: queue
      ...
      - alert: function1
        ...
        annotations:
          summary: Queue Overflowed, current queue length is {{ $value }}
          description: "[{{ $labels.job }}] 队列阻塞，当前排队数为 {{ $value }}"  # 告警信息
      ...
      - alert: function2
        ...
        labels:
          ...
          level: 2   #告警级别,告警级别定义 0 信息,1 警告,2 一般严重,3 严重,4 灾难
        annotations:
          summary: Job failure warning, current failed jobs is {{ $value }}
          description: "[{{ $labels.job }}] 当前失败任务数为 {{ $value }}"  # 告警信息
          wxurl: https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=xxxx
  ...
  alertmanager.yml: |-
    global:
      ...
      smtp_from: '361463172@qq.com'
      smtp_auth_username: '361463172'
    ...
    - name: 'prometheusalert-all'
      webhook_configs:
      - url: 'http://prometheus-alert-center.monitor-sa:8080/prometheus/alert'
        send_resolved: true
  ...
  template:
    ...
    spec:
      ...
      - name: prometheus
        ...
        - mountPath: /etc/prometheus/alertrules.yml
          name: prometheus-config
          subPath: alertrules.yml
      ...
      volumes:
        ...
        - name: prometheus-storage-volume
          hostPath:
           path: /data/arena/prom
           type: Directory
        - name: alertmanager-storage
          hostPath:
           path: /data/arena/alertmanager
...
### 部署PrometheusAlertCenter
```prometheus-alert-center.yaml
apiVersion: v1
data:
  app.conf: |
    #---------------------↓全局配置-----------------------
